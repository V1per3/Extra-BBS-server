...
const post ={
  ...
  //管理运营获取帖子列表
  async getPostListForAdmin(pageNum, pageSize, topicId, sort, keyword, status){
    //判断话题是否是父级
    let topic = await topicModel.getTopic(topicId)
    let sid
    if(topic.sid == 0){
    ...
    let postList = await postModel.getPostListForAdmin(pageNum, pageSize, topicId, sort, sid, keyword, status)
    if(postList){
      return {
        status: 200,
        message: postList
