...
const user = {
  ...
  async verify(email){
    ...
    let mailOptions = {
      from: Email.user,
      to: ko.email,
      subject: 'Extra BBS.live 社区注册验证码',
      html:  `<div style="background-color: #ffffff">
                <div style="margin: auto; width: 500px; height: auto; padding: 20px; position: relative; background-color: #f8f8f8">
                  ...
                  <div style="padding: 5px 20px 20px;">
                      <p style="font-size: 16px;">感谢您使用 Extra BBS.live 服务</p>
                      <p style="font-size: 16px;">您的邮箱注册验证码是： <span style="color: #009688; font-weight: bold;">${ko.code}</span></p>
                  </div>
                  <div>此为系统邮件，请勿回复</div>
                  <div>©2022 <a href="Extra BBS.live" style="cursor: pointer; color: #409eff;">Extra BBS</a></div>
                </div> 
              </div>`
    };
    //发送邮件
